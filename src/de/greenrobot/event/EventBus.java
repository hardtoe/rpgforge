...
public class EventBus {
    ...
    private final WeakHashMap<Object, List<Class<?>>> typesBySubscriber;
    ...
    public EventBus() {
        ...
        typesBySubscriber = new WeakHashMap<Object, List<Class<?>>>();
    ...
    private synchronized void register(
        ...
        final boolean sticky, Class<?> eventType,
        final Class<?>... moreEventTypes
    ) {
        final Class<?> subscriberClass = 
            subscriber.getClass();
        
        final List<SubscriberMethod> subscriberMethods = 
            subscriberMethodFinder.findSubscriberMethods(subscriberClass, methodName);
    ...
    private void unubscribeByEventType(Object subscriber, Class<?> eventType) {
        ...
        if (subscriptions != null) {
            ...
            for (int i = 0; i < size; i++) {
                if (
                    subscriptions.get(i).getSubscriber() == null || 
                    subscriptions.get(i).getSubscriber() == subscriber
                ) {
    ...
    void invokeSubscriber(Subscription subscription, Object event) throws Error {
        try {
            subscription.getSubscriberMethod().method.invoke(subscription.getSubscriber(), event);
            ...
            if (event instanceof SubscriberExceptionEvent) {
                ...
                SubscriberExceptionEvent exEvent = 
                    new SubscriberExceptionEvent(this, cause, event, subscription.getSubscriber());
                post(exEvent);
            }
